 ...
 * @note 未完成说明：
 *          本文件未完成部分用"TODO:"进行了标识。未完成内容：
 *          消耗品的use方法未实现、用具equipAndUnequip方法未适配主角类接口的改变、json文件数据不可强转为string的问题未解决
 * */
...
unique_ptr<Item> ItemManager::createItem(string item_name) {
    ...
    */
    ...
    case food:
        ...
        * @brief 创建物品对象并返回
        */
        unique_ptr<Item> item_ptr = make_unique<Food>(name，description, value, strength_restore, health_restore, time_cooldown);
        return item_ptr;
    ...
    case learning_aid:
        ...
        * @brief 创建物品对象并返回
        */
        unique_ptr<Item> item_ptr = make_unique<LearningAid>(name，description, value, intel_boost, intel_boost_rate, duration, have_abuse_punish, health_reduce);
        return item_ptr;
    ...
    case health_item:
        ...
        * @brief 创建物品对象并返回
        */
        unique_ptr<Item> item_ptr = make_unique<LearningAid>(name，description, value, health_restore, duration);
        return item_ptr;
