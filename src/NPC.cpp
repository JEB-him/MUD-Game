...
void NPC::handleInteraction(int optionIndex) const {
    /// @brief 处理具体交互选项（可被子类重写）
...
/// @brief 获取交互文本
std::string NPC::getInteractionText() const { return interactionText; }
/// @brief 获取交互选项
std::vector<std::string> NPC::getInteractionOptions() const { return interactionOptions; }

/// @brief 设置称呼
void NPC::setTitle(const std::string& newTitle) { title = newTitle; }
/// @brief 设置身份
void NPC::setIdentity(const std::string& newIdentity) { job = newIdentity; }
/// @brief 设置交互文本
void NPC::setInteractionText(const std::string& text) { interactionText = text; }
/// @brief 添加交互选项
void NPC::addInteractionOption(const std::string& option) { interactionOptions.push_back(option); }
 ...
 * */
void Student::handleInteraction(int optionIndex) const {
...
* */
void SeniorStudent::handleInteraction(int optionIndex) const {
...
/// @brief 设置专业
void SeniorStudent::setMajor(const std::string& newMajor) { major = newMajor; }
 ...
 * @param optionIndex 交互选项索引
 * */
void Roommate::handleInteraction(int optionIndex) const {
    switch (optionIndex) {
        case 0: // 开黑打游戏
 ...
 * @return 带饭费用
 */
int Roommate::getMealCost() const { return mealCost; }
 ...
 */
void DormManager::handleInteraction(int optionIndex) const {
    switch (optionIndex) {
        case 0: // 对话
 ...
 * @brief 招聘人员类构造函数
 * */
Employer::Employer(const std::string& title, int moneyReward, int staminaCost, 
 ...
 * @brief 处理与招聘人员的交互
 * */
void Employer::handleInteraction(int optionIndex) const {
    switch (optionIndex) {
        case 0: // 接受工作
 ...
 * @brief 处理与图书管理员的交互
 * */
void Librarian::handleInteraction(int optionIndex) const {
    if (optionIndex == 0) {
 ...
 * @brief 处理与食堂工作人员的交互
 * */
void CanteenStaff::handleInteraction(int optionIndex) const {
    switch (optionIndex) {
        case 0: // 查看菜单
        ...
        case 2: // 为舍友买饭
            ...
            if (roommateFoodChoice > 0 && roommateFoodChoice <= static_cast<int>(foodTypes.size())) {
                ...
                std::cout << "体力恢复：+" << staminaRecovery[roommateFoodChoice-1] << "\n";
 ...
 * @brief 处理与教师的交互
 * */
void Professor::handleInteraction(int optionIndex) const {
    switch (optionIndex) {
        case 0: // 请教问题
 ...
 * @brief 教练员类构造函数
 * */
Coach::Coach(const std::string& title, const std::string& sportType)
 ...
 * @brief 处理与教练员的交互
 * */
void Coach::handleInteraction(int optionIndex) const {
    switch (optionIndex) {
        case 0:
...
void Coach::handleTrainingOptions() const {
    ...
    for (const auto& sport : sportsData) {
        if (sport.name == choice) {
            ...
            int playerGold = 100; // 假设从主角类获取金币数量，这里用100作为示例
            
            std::cout << "您选择了" << choice << "训练！\n";
            std::cout << "将花费：" << sport.cost << "金币\n";
            std::cout << "将获得：体力+" << sport.staminaGain << "\n";
            std::cout << "将消耗时间：" << sport.timeCost << "小时\n";
            
            if (playerGold >= sport.cost) {
                std::cout << "金币足够，训练成功！\n";
                Coach::handleTrainingReward(sport);
            } else {
                std::cout << "金币不足！需要" << sport.cost << "金币，但您只有" << playerGold << "金币。训练失败！\n";
            }
            return;
        }
    }
...
void Coach::initializeSportsData() {
    ...
    if (!file.is_open()) {
        ...
        // 使用默认数据作为后备
        sportsData.push_back({"篮球", 0.5, 15, 5});
        sportsData.push_back({"足球", 1.0, 15, 8});
        sportsData.push_back({"跑步", 1.0, 0, 5});
        sportsData.push_back({"羽毛球", 0.5, 35, 10});
        return;
    }
    ...
    try {
        ...
        // 解析篮球数据
        if (sportsJson.contains("basketball")) {
            auto& basketball = sportsJson["basketball"];
        ...
        // 解析足球数据
        if (sportsJson.contains("football")) {
            auto& football = sportsJson["football"];
        ...
        // 解析跑步数据
        if (sportsJson.contains("running")) {
            auto& running = sportsJson["running"];
            sportsData.push_back({"跑步",
        ...
        // 解析羽毛球数据
        if (sportsJson.contains("badminton")) {
            auto& badminton = sportsJson["badminton"];
 ...
 * @return 运动项目类型
 * */
std::string Coach::getSportType() const { return sportType; }
